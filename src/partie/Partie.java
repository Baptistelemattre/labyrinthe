	        	...
	        	for (int i = 0; i < elementsPartie.getNombreJoueurs(); i++) {
	            	...
	            	//Verification si le joueur est sur un objet, si oui l'enleve du plateau et confirme qu'il a trouve l'objet
		            	for(int j=0;j<elementsPartie.getJoueurs()[i].getObjetsJoueur().length;j++) {
		            		if( elementsPartie.getJoueurs()[i].getObjetsJoueur()[j].getPosLignePlateau() == posJoueur[0] && elementsPartie.getJoueurs()[i].getObjetsJoueur()[j].getPosColonnePlateau() == posJoueur[1] && elementsPartie.getJoueurs()[i].getProchainObjet() == elementsPartie.getJoueurs()[i].getObjetsJoueur()[j])  {//V�rification si Joueur sur un de ces objets
